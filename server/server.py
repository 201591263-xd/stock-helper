...
def _get_cyb_pool():
    """创业板股票池：多样化标的，避免全是龙头，控制股价区间"""
    return {
        '科技': [('300373','扬杰科技','功率半导体中盘'), ('300433','蓝思科技','消费电子中盘'), ('300136','信维通信','射频天线中盘')],
        'AI': [('300418','昆仑万维','AI+游戏出海'), ('300229','拓尔思','NLP大数据'), ('300624','万兴科技','AI创意工具中盘')],
        '消费': [('300908','仲景食品','调味品'), ('300755','华致酒行','酒类流通'), ('300783','三只松鼠','休闲零食')],
        '新能源': [('300568','星源材质','隔膜中盘'), ('300450','先导智能','锂电设备中盘'), ('300073','当升科技','正极材料中盘')],
        '医药': [('300363','博腾股份','CDMO弹性小盘'), ('300347','泰格医药','CRO中盘'), ('300759','康龙化成','CRO中盘')],
        '金融': [('300773','拉卡拉','支付中盘'), ('300803','指南针','券商IT中盘'), ('300033','同花顺','AI金融中盘')],
        '军工': [('300775','三角防务','航空锻件中盘'), ('300699','光威复材','碳纤维中盘'), ('300034','钢研高纳','高温合金中盘')],
        '周期': [('300618','寒锐钴业','钴中盘'), ('300505','川金诺','磷化工小盘'), ('300437','清水源','水处理小盘')],
    }
...
def call_hermes(sector: str, exclude_codes: list, board_filter: str = '', pool=None) -> dict:
    ...
    # 过滤：股价超过100的不推
    scored_candidates = [(c, n, r, p, chg) for c, n, r, p, chg in scored_candidates if p <= 100]
    if not scored_candidates:
        return None  # 全部超100元，该板块放弃
...
你必须基于短线交易核心体系选票，严格按以下框架执行：

【第一步：判断情绪周期】（从候选票涨跌幅分布推断）
- 冰点/退潮期（多数跌幅>2%）：防御为主，选中盘稳健标的或回调到位的龙头，仓位2成
- 回暖期（多数微涨0-2%）：均衡配置，龙头和中盘各半，打板+低吸并行
- 高潮期（多票涨幅>5%）：激进选还没涨的弹性小票，已拉高的龙头不追，可重仓

【第二步：五维共振检验】至少满足3个维度才可推荐：
1. 大盘：候选票整体涨多跌少则大盘健康
2. 板块：若板块内多票上涨则板块共振强
3. 个股：涨幅1-5%最优，换手活跃（8-20%），均线多头
4. 消息：有利好催化或资金关注的优先
5. 资金：主力净流入或大单买入的优先

【第三步：分歧一致判断】
- 买在分歧：换手8-20%、盘中烂板回封、非一字板 → 优先
- 卖在一致：缩量秒板、一字板、板块高潮追高 → 避开
...
【选票优先级】
1. 涨幅适中1-5% > 涨幅过大>7% > 跌幅票
2. 中小盘弹性票优先于大盘龙头
3. 分歧票优先于一致票
4. 避免近期重复

你不是龙头推荐机器——用脑子选票，每次独立判断。
